...
function TeamsPage() {
    ...
    const HandleAdmin = async () => {
        const response = await fetch('https://ticketsystem-backend-8vh3.onrender.com/api/memberlogin/All_ADMIN_data', {
            method: "GET",
            headers: {
    ...
    const MemberDelete = async (id) => {
        const response = await fetch(`https://ticketsystem-backend-8vh3.onrender.com/api/adminlogin/deletemember/${id}`, {
            method: "DELETE",
            headers: {
                'Content-type': 'application/json'
            }
